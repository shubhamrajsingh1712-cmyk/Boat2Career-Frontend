...
export default function AssessmentPage() {
  ...
  }, [testType, navigate]); // 👈 dependencies used inside
  ...
  const handleSubmit = async () => {
    ...
    if (answeredCount < questions.length) {
      ...
      toast.warning(
        `You have ${unansweredCount} unanswered question(s). Submitting anyway...`,
      );
    }
    ...
    try {
      ...
      toast.success(
        `Assessment submitted! You answered ${formattedResponses.length} out of ${questions.length} questions.`,
      );
      navigate("/dashboard");
    } catch (error) {
        ...
        <Card className="card-base p-8 mb-6">
          ...
          <div className="space-y-4">
            <RadioGroup
              ...
              onValueChange={(val) => {
                console.log(
                  `Selected value: ${val} for question: ${question.id}`,
                );
